...
color_5 = (0, 255, 0)  # green color (fixed)
color_6 = (255, 0, 0)  # red color

box_length = 900
box_height = 600

add_caption = pygame.display.set_mode((box_length, box_height))  # Fixed tuple
...
def make_snake(snake_block, list_snake):
    for x in list_snake:
        pygame.draw.rect(
            add_caption, color_3, [x[0], x[1], snake_block, snake_block]
        )  # Fixed missing width/height
...
def game_start():
    ...
    while not game_over:
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_c:
                        return game_start()  # Restart the game properly
        ...
        value_x1 += new_x1
        value_y1 += new_y1
        add_caption.fill(color_1)  # Changed to white for better contrast

        pygame.draw.rect(
            add_caption, color_5, [foodx_pos, foody_pos, snake_block, snake_block]
        )  # Fixed food position
        ...
        make_snake(snake_block, list_snake)  # Fixed missing snake rendering
